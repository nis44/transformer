...
class BilingualDataset(Dataset):
    ...
    def __getitem__(self, index: Any) -> Any:
        ...
        if enc_num_padding_tokens < 0 or dec_num_padding_tokens < 0:
            raise ValueError(f"Sequence length {self.seq_len} is too short for the given input. Consider increasing the sequence length.")
        ...
        return {
            ...
            "decoder_attention_mask": (decoder_input != self.pad_token).unsqueeze(0).unsqueeze(0).int() & causal_mask(decoder_input.size(0)),
            'src_text': src_text,
            'tgt_text': tgt_text,
            'label': label
        }
...
def causal_mask(size):
    mask = torch.tril(torch.ones(size, size)).unsqueeze(0).unsqueeze(0).int()
    return mask     